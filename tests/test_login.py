...
def test_invalid_login(test_client, init_database):
    ...
    response = test_client.post(
        "/login", json={"username": correct_username, "password": fake_password}
    )
    assert response.status_code == 401
    assert response.json["error"] == "wrong username or password"

    response = test_client.post(
        "/login", json={"username": fake_username, "password": correct_password}
    )
    assert response.status_code == 401
    assert response.json["error"] == "wrong username or password"

    response = test_client.post(
        "/login", json={"username": fake_username, "password": fake_password}
    )
    assert response.status_code == 401
    assert response.json["error"] == "wrong username or password"
