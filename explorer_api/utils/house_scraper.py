...
def scrape_house_info(city, street_type, street, house_type, house, block_type, block):
    ...
    page_count = len([el for el in soup.find("ul", class_="pagination").contents if el != "\n"]) - 1
    if page_count > 1:
        for page in range(2, page_count+1):
            ...
            page_houses_table = page_soup.find("div", class_='table-responsive').contents[1]
            page_table_head, page_table_body = [child for child in page_houses_table.children if child != "\n"]
